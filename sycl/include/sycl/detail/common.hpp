...
/// Emits an XPTI trace before a SYCL API call is made
/// \param Data
/// \return The correlation ID for the API call that is to be used by the
/// emitFunctionEndTrace() call
uint64_t emitFunctionBeginTrace(const char *Data);

/// Emits an XPTI trace after the SYCL API call has been made
/// \param CorrelationID The correlation ID for the SYCL API call generated by the
/// emitFunctionBeginTrace() call.
/// \param FName The name of the SYCL API call
...
class TraceFunction
{
  TraceFunction(const char *FName, ArgsT... Args) : MFName(FName)
  {  
    MCorrelationID = emitFunctionBeginTrace(MFName.c_str());
  }
  ~TraceFunction()
  {
    emitFunctionEndTrace(MCorrelationID, MFName.c_str());
  }
private:
  std::string MFName;
